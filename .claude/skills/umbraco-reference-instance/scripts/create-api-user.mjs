...
// Vendored verbatim (bar this comment) from the v17 line of the Umbraco CMS MCP Dev repo:
//   https://github.com/umbraco/Umbraco-CMS-MCP-Dev/blob/v17/main/scripts/create-api-user.mjs
// Copied here because the repo ships this script in git only — it is not in the published
// @umbraco-cms/mcp-dev package, so there is nothing to npx.
//
// Take it from the v17 branch, NOT main/dev: those track Umbraco 18, where the Swagger OAuth
// callback moved to /umbraco/openapi/oauth2-redirect.html. Umbraco 17 registers
// /umbraco/swagger/oauth2-redirect.html and rejects the v18 path with an invalid redirect_uri
// error, surfaced unhelpfully as "No redirect from authorize endpoint". Re-sync from the branch
// matching the instance's major (see Directory.Packages.props), not from the default branch.
//
// Run it via `instance.sh api-user`, which passes this repo's URL/credentials and handles the
// dev-certificate TLS exemption.
